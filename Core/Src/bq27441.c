...
static bool BQ27441_writeExtendedData(uint8_t classID, uint8_t offset,
                                      uint8_t *data, uint8_t len);

static volatile bool sealFlag =
    false; // Global to identify that IC was previously sealed
static volatile bool userConfigControl =
    false; // Global to identify that user has control over
...
/**
 * Configures terminate voltage (lowest operational voltage of battery powered
 * circuit)
 *
...
bool BQ27441_exitConfig(bool resim) {
  ...
  if (resim) {
    if (BQ27441_softReset()) {
      ...
      if (timeout > 0) {
        if (sealFlag)
          BQ27441_seal(); // Seal back up if we IC was sealed coming in
        return true;
      }
    }
    return false;
 ...
 * */
uint16_t BQ27441_flags(void) { return BQ27441_readWord(BQ27441_COMMAND_FLAGS); }
...
static uint8_t BQ27441_readExtendedData(uint8_t classID, uint8_t offset) {
  uint8_t retData = 0;
  if (!userConfigControl)
    BQ27441_enterConfig(false);
  ...
  retData =
      BQ27441_readBlockData(offset % 32); // Read from offset (limit to 0-31)
