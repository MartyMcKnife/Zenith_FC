...
W25QXX_result_t w25qxx_read(W25QXX_HandleTypeDef *w25qxx, uint32_t address,
  ...
  uint8_t tx[4] = {W25QXX_READ_DATA, (uint8_t)(address >> 16),
...
W25QXX_result_t w25qxx_write(W25QXX_HandleTypeDef *w25qxx, uint32_t address,
  ...
  for (uint32_t page = first_page; page <= last_page; ++page) {
    ...
    uint32_t write_len =
        w25qxx->page_size - (start_address & (w25qxx->page_size - 1));
    write_len = len > write_len ? write_len : len;

    W25_DBG("w25qxx_write: handling page %lu start_address = 0x%08lx "
            "buffer_offset = 0x%08lx len = %04lx",
            page, start_address, buffer_offset, write_len);
    ...
    if (w25qxx_write_enable(w25qxx) == W25QXX_Ok) {
      ...
      if (w25qxx_transmit(w25qxx, tx, 4) ==
        ...
        if (w25qxx_transmit(w25qxx, buf + buffer_offset, write_len) !=
            W25QXX_Ok) {
          cs_off(w25qxx);
          return W25QXX_Err;
        }
